...
func TestFindInterfaceByName(t *testing.T) {
	...
	testIface := ifaces[0]
	lookupName := getInterfaceName(testIface)

	// Test finding by name
	endpoint, err := findInterfaceByName(lookupName, ifaces)
	if err != nil {
		t.Errorf("findInterfaceByName() error = %v", err)
	}
	if endpoint != nil && endpoint.Name() != lookupName {
...
func TestFindInterface(t *testing.T) {
	...
	if err == nil && len(ifaces) > 0 {
		lookupName := getInterfaceName(ifaces[0])
		endpoint, err = FindInterface(lookupName)
		if err != nil {
...
func TestGatewayProvidedByUser(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			endpoint, err := GatewayProvidedByUser(iface, tt.gateway)

			if tt.expectValidationError {
				if err == nil {
					t.Error("GatewayProvidedByUser() should reject invalid gateway input")
				}
				return
			}

			if err == nil && endpoint == nil {
				t.Error("GatewayProvidedByUser() returned nil endpoint without an error")
			}

			if err != nil && strings.Contains(err.Error(), "not a valid IPv4") {
				t.Errorf("GatewayProvidedByUser() rejected valid IPv4 input: %v", err)
			}
		})
	}
}
