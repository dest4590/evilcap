...
import (
	...
	"github.com/dest4590/evilcap/v2/core"
...
func ParseTargets(targets string, aliasMap *data.UnsortedKV) (ips []net.IP, macs []net.HardwareAddr, err error) {
	...
	for _, part := range strings.Split(targets, ",") {
		...
		// is it a MAC?
		if macParser.MatchString(part) {
			normalizedMac := NormalizeMac(part)
			if hw, err := net.ParseMAC(normalizedMac); err == nil {
				macs = append(macs, hw)
				continue
			}
		}
		...
		// is it a range?
		if list, err := iprange.ParseList(part); err == nil {
			ips = append(ips, list.Expand()...)
			continue
		}

		return nil, nil, fmt.Errorf("could not resolve target %s", part)
	}
