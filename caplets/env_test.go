...
func TestGetDefaultInstallBase(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		expected := filepath.Join(os.Getenv("ALLUSERSPROFILE"), "evilcap")
		if base != expected {
			t.Errorf("on windows, expected %s, got %s", expected, base)
		}
	} else {
		expected := "/usr/local/share/evilcap/"
		if base != expected {
...
func TestConstants(t *testing.T) {
	...
	if InstallArchive != "https://github.com/dest4590/evilcap/archive/master.zip" {
