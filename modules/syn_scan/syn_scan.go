...
func NewSynScanner(s *session.Session) *SynScanner {
	...
	mod.AddParam(session.NewBoolParameter("syn.scan.fast",
		"false",
		"If true, only common ports will be scanned."))

	mod.AddParam(session.NewBoolParameter("syn.scan.stealth",
		"false",
		"If true, a random delay will be added between probes to make the scan less predictable."))
...
func (mod *SynScanner) Configure() (err error) {
	...
	if err, mod.fast = mod.BoolParam("syn.scan.fast"); err != nil {
		return err
	} else if err, mod.stealth = mod.BoolParam("syn.scan.stealth"); err != nil {
		return err
	}
...
func (mod *SynScanner) scanWorker(job async.Job) {
	...
	for _, dstPort := range mod.ports {
		...
		if mod.stealth {
			// add a random delay between 10 and 100 ms
			delay := 10 + rand.Intn(90)
...
func (mod *SynScanner) synScan() error {
	...
	mod.SetRunning(true, func() {
		...
		if mod.fast {
			...
			mod.ports = make([]int, 0)
			for p := mod.startPort; p <= mod.endPort; p++ {
				mod.ports = append(mod.ports, p)
			}
		}

		mod.stats.numPorts = uint64(len(mod.ports))
