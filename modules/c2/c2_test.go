...
func TestNewC2(t *testing.T) {
	...
	if mod.settings.nick != "evilcap" {
		t.Errorf("Expected default nick 'evilcap', got '%s'", mod.settings.nick)
	}

	if mod.settings.user != "evilcap" {
		t.Errorf("Expected default user 'evilcap', got '%s'", mod.settings.user)
	}
